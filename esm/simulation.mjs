// esm/simulation.mjs — ES module simulation (browser-friendly)
import { clamp, avgStat, sumStat } from './helpers.mjs';

function installedIds(state, roomIndex, category) {
...
export function simulateRecording(state, roomIndex, contract) {
  ...
  const itemsById = state.itemsById instanceof Map ? state.itemsById : new Map((db.items||[]).map(i=>[i.id,i]));
  ...
  const fatigue_penalty = Math.max(0, (state.player && state.player.fatigue || 0) - 10) * 2;
...
// Expose for progressive migration
if (typeof window !== 'undefined') {
  window.ESSimulation = { simulateRecording };
}

export default { simulateRecording };
